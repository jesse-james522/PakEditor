...
public partial class App
{
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        if (!Directory.Exists(UserSettings.Default.OutputDirectory))
        {
            ...
            {
                throw new Exception("PakEditor cannot create the output directory where it is currently located. Please move PakEditor.exe to a different location.", exception);
            }
        }
        ...
        Directory.CreateDirectory(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "PakEditor"));
        ...
        Log.Logger = new LoggerConfiguration()
            ...
            .WriteTo.File(outputTemplate: template,
                path: Path.Combine(UserSettings.Default.OutputDirectory, "Logs", $"PakEditor-Debug-Log-{DateTime.Now:yyyy-MM-dd}.log"))
#else
            .Enrich.With<CallerEnricher>()
            .WriteTo.File(outputTemplate: template,
                path: Path.Combine(UserSettings.Default.OutputDirectory, "Logs", $"PakEditor-Log-{DateTime.Now:yyyy-MM-dd}.log"))
#endif
            .CreateLogger();
