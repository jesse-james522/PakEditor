...
public class CUE4ParseViewModel : ViewModel
{
    ...
    public CurveViewModel CurveVM { get; } = new();
    ...
    private bool CheckExport(CancellationToken cancellationToken, IPackage pkg, int index, EBulkType bulk = EBulkType.None) // return true once you want to stop searching for exports
    {
        ...
        switch (dummy)
        {
            ...
            case UCurveLinearColor when isNone && pointer.Object.Value is UCurveLinearColor colorCurve:
            {
                CurveVM.LoadColorCurve(colorCurve, pkg.Name);
                return false; // fall through to also show JSON
            }
            case UCurveFloat when isNone && pointer.Object.Value is UCurveFloat floatCurve:
            {
                CurveVM.LoadFloatCurve(floatCurve, pkg.Name);
                return false;
            }
            case UDataTable when isNone && pointer.Object.Value is UDataTable dataTable:
            {
                CurveVM.LoadDataTableCurves(dataTable, pkg.Name);
                return false;
            }
            default:
            {
